...
def main():
    ...
    print("Loading CIFAR-100 Coarse features via Pretrained ResNet18...")
    # This will download ~160MB
    X, y = get_image_dataset("cifar100", pretrained=True, device=device.type)
    ...
    for ep in range(1, cfg['training']['epochs'] + 1):
        train_epoch(model, loader, opt_enc, opt_proto, opt_mass, device, 
                    ...
                    use_divergence=True)
        if ep % 5 == 0:
            print(f"  Epoch {ep:2d}/{cfg['training']['epochs']} completed")
    ...
    # 3. DeepCluster-v2
    print("\n--- DeepCluster-v2 ---")
    from src.eval.deep_baselines import DeepClusterV2Baseline
    dc2 = DeepClusterV2Baseline(n_clusters=num_classes)
    dc2.fit(X.numpy())
    metrics_dc2 = compute_all_metrics(y.numpy(), dc2.labels_)
    print(f"DeepCluster-v2 | ACC: {metrics_dc2['ACC']:.3f} | ARI: {metrics_dc2['ARI']:.3f}")
    results['deepcluster-v2'] = {'metrics': metrics_dc2}

    # 4. DINO Clustering Proxy
    print("\n--- DINO Clustering ---")
    from src.eval.deep_baselines import DINOClusteringBaseline
    dino = DINOClusteringBaseline(n_clusters=num_classes)
    dino.fit(X.numpy())
    metrics_dino = compute_all_metrics(y.numpy(), dino.labels_)
    print(f"DINO | ACC: {metrics_dino['ACC']:.3f} | ARI: {metrics_dino['ARI']:.3f}")
    results['dino'] = {'metrics': metrics_dino}

    # Output to YAML
    out_path = "experiments/results/cifar100_summary.yaml"
